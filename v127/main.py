...
class PDUA(Adw.Application):
    ...
    def _on_auth(self, action, param):
        ...
        auth_win = AuthWindow(parent)

        def on_login_success(window):
            log.info("login succeeded")

        active = self.props.active_window
        if active and hasattr(active, "refresh_auth_state"):
            active.refresh_auth_state()

        auth_win.connect("login-success", on_login_success)
        auth_win.present()
